angular.module('main').controller('McqExamCtrl',function ($scope,$http,$rootScope,toastr,$state,DTOptionsBuilder, DTColumnBuilder,$compile,mySocket,$localForage) {
	...
	$scope.exam = function(){
		// mySocket.emit('demo',{"data":"test"});
		var temp;
		var exam = [];
		console.log($rootScope.examQuestion);
		$localForage.setItem('examQuestion',$rootScope.examQuestion);
		$state.go('admin.exam');
		// $localForage.getItem('examUser').then(function(data){
		// 	console.log(data);
		// 	console.log("Local Sotorage call");
		...
		// 	for(var i = 0; i < data.length; i++){
		// 		// temp = $rootScope.examQuestion.shuffle();
		// 		data[i].question = _.shuffle($rootScope.examQuestion);
		// 		console.log(temp);
		// 		exam.push(data[i]);
		// 		temp = null;
		// 		console.log(exam[i].question[0]);
		// 	}
		// 	mySocket.emit('exam',{data:exam});
		// 	console.log(exam);
		// });
	}
});